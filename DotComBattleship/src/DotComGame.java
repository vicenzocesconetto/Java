...
class DotComGame {
    ...
    private ArrayList<String> deployDotCom() {
        ...
        while (!success) {
            ...
            if(gridLocations[position[0]] != 0) {               // if this position is available
                ...
                if(position[1] < gridSize && gridLocations[position[1]] != 0 && (position[0] % gridWidth <= position[1] % gridWidth) ) {
                    position[2] = position[1] + increment;

                    if (position[2] < gridSize && gridLocations[position[2]] != 0 && (position[1] % gridWidth <= position[2] % gridWidth)  ) {
                        success = true;
    ...
    private void checkHit(String userGuess) {
        ...
        for (DotComShip current : dotComList) {
            ...
            if(hitmark.equals("kill")) {
                dotComList.remove(current);
                return;
            }
            if(hitmark.equals("hit"))
                return;
        }
        if(hitmark.equals("miss"))
